...
export default function Room() {
  ...
  const confirmFigure = useCallback((serverData: ServerFigureEvent) => {
    ...
    const matchIdx = pendingRef.current.findIndex((p) =>
      figuresMatch(p, incoming),
    );
    if (matchIdx !== -1) {
  ...
  const handlePointerUp = () => {
    ...
    if (
      ...
      tool === "rectangle" &&
      (previewShape?.width ?? 0) > 4 &&
      (previewShape?.height ?? 0) > 4
    ) {
      const figure: Figure = {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden">
      ...
      <div className="fixed top-1 left-2 right-2 flex items-center justify-between px-5 py-3.5 pointer-events-none z-[100]">
        <button
          onClick={() => navigate("/")}
